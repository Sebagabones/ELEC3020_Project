...
// #define TFT_RST  -1     // Set TFT_RST to -1 if the display RESET is
...
// #define LOAD_FONT8  // Font 8. Large 75 pixel font needs ~3256 bytes in
// FLASH, only characters 1234567890:-.
...
// divider ratio may need to be increased when overclocking. It may also need to
// be adjusted dependant on the display controller type (ILI94341, HX8357C
// etc.). If RP2040_PIO_CLK_DIV is not defined the library will set default
// values which may not suit your display. The display controller data sheet
// will specify the minimum write cycle period. The controllers often work
// reliably for shorter periods, however if the period is too short the display
// may not initialise or graphics will become corrupted. PIO write cycle
// frequency = (CPU clock/(4 * RP2040_PIO_CLK_DIV))
