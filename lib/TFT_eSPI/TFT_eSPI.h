...
// The following lines allow the user setup to be included in the sketch folder,
// see "Sketch_with_tft_setup" generic example.
...
/***************************************************************************************
**                         Section 6.2: Colour enumeration
***************************************************************************************/
// Catppuccin colours (Mocha)
...
public:
  ...
  void
  setRotation(uint8_t r); // Set the display image orientation to 0, 1, 2 or 3
...
// switched to input
  ...
  // return blended colour If the bg_color is not specified, the background
  ...
  // Draw an anti-aliased (smooth) arc between start and end angles. Arc ends
  // are anti-aliased. By default the arc is drawn with square ends unless the
  // "roundEnds" parameter is included and set true Angle = 0 is at 6 o'clock
  // position, 90 at 9 o'clock etc. The angles must be in range 0-360 or they
  ...
  // effect of anti-aliasing narrow lines
  ...
  // Draw a rounded rectangle that has a line thickness of r-ir+1 and bounding
  ...
  int16_t
  getCursorX(void), // Read current cursor x position (moves with tft.print())
  ...
  void setTextColor(uint16_t color), // Set character (glyph) color only
      ...
      setTextColor(uint16_t fgcolor, uint16_t bgcolor,
                   bool bgfill = false), // Set character (glyph) foreground and
...
#else
  void
  setFreeFont(uint8_t font),     // Not used, historical fix to prevent an error
      setTextFont(uint8_t font); // Set the font number to use in future
#endif
  ...
  uint16_t
  fontsLoaded(void); // Each bit in returned value represents a font type that
                     // is loaded - used for debug/error handling only

  // Low level read/write
  void spiwrite(uint8_t); // legacy support only
#ifdef RM68120_DRIVER
  void writecommand(uint16_t c); // Send a 16-bit command, function resets DC/RS
                                 // high ready for data
  void
  writeRegister8(uint16_t c,
                 uint8_t d); // Write 8-bit data data to 16-bit command register
  void writeRegister16(
      uint16_t c,
      uint16_t d); // Write 16-bit data data to 16-bit command register
#else
  void writecommand(uint8_t c); // Send an 8-bit command, function resets DC/RS
                                // high ready for data
#endif
  ...
  // These can be used for SPI writes when using the ESP32 (original) or STM32
  // processors. DMA also works on a RP2040 processor with PIO based SPI and
  // parallel (8 and 16-bit) interfaces Bear in mind DMA will only be of benefit
  // in particular circumstances and can be tricky to manage by noobs. The
  ...
  bool initDMA(
      bool ctrl_cs =
          false); // Initialise the DMA engine and attach to SPI bus - typically
                  // used in setup() Parameter "true" enables DMA engine control
                  // of TFT chip select (ESP32 only) For ESP32 only, TFT reads
                  // will not work if parameter is true
  ...
  // over-written or destroyed while DMA is in progress
  //
  // Note 1: If swapping colour bytes is defined, and the double buffer option
  // is NOT used, then the bytes in the original image buffer content will be
  // byte swapped by the function before DMA is initiated.
  //
  // Note 2: If part of the image will be off screen or outside of a set
  // viewport, then the the original image buffer content will be altered to a
  // correctly clipped image before DMA is initiated.
  //
  // The function will wait for the last DMA to complete if it is called while a
  // previous DMA is still in progress, this simplifies the sketch and helps
  // avoid "gotchas".
...
#define PSRAM_ENABLE 3
  ...
  // Section 7 above
  void
  getSetup(setup_t &tft_settings); // Sketch provides the instance to populate
...
private:
  ...
  uint32_t xset_mask[256]; // Makes Sprite rendering test 33% faster, for slower
                           // macro equivalent see commented out #define
...
protected:
  ...
  bool _utf8;  // If set, use UTF-8 decoder in print stream 'write()' function
               // (default ON)
  bool
      _psram_enable; // Enable PSRAM use for library functions (TBD) and Sprites

  uint32_t _lastColor; // Buffered value of last colour used

  bool
      _fillbg; // Fill background flag (just for for smooth fonts at the moment)
...
static inline uint16_t fastBlend(A alpha, F fgc, B bgc) {
